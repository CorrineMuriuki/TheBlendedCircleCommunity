...
export function FeatureCard({ icon, title, description }: FeatureCardProps) {
  return (
    <div className="p-6 sm:p-8 rounded-xl bg-white shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border border-neutral-100">
      ...
      <h3 className="text-sm sm:text-base font-medium mb-2 text-[#9e7a68] text-center sm:text-left">{title}</h3>
      <p className="text-gray-700 text-xs sm:text-sm leading-relaxed text-center sm:text-left">{description}</p>
    </div>
  );
}